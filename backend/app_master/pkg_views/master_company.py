...
class Company(View):
    ...
    def post(self, request, pk=None):
        ...
        if company_de_serialized.is_valid():
            ...
            payload = super().create_payload(
                success=False,
                message=f"{C_SERIALIZING_ERROR} : {company_de_serialized.errors}",
            )
            return Response(data=payload, status=status.HTTP_400_BAD_REQUEST)
            ...
            for data in request.data[self.C_BATCH]:
                ...
                if company_de_serialized.is_valid():
                    try:
                        ...
                        _message.append(f"{Company().get_view_name()} {C_EXISTS}")
                    ...
                    _message.append(
                        f"{C_SERIALIZING_ERROR} : {company_de_serialized.errors}"
                    )
